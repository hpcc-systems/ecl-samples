/*
   The ML.Classify.DecisionTree.GiniImpurityBased.TestD function
   uses indep and dep to first create a model using LearnD.
   Then, it uses ClassifyD to compute estimates of the dependent
   variables given indep and the model. And finally,
   it compares dep and the computed estimates of dep using
   Classify.Compare.
...
*/
   ...
   comparison:= ML.Classify.DecisionTree.GiniImpurityBased(5, 1).TestD(Indep, Dep);
   OUTPUT(comparison);
