/*
   The ML.Classify.Logistic.ClassifyC, which redefines the inherited
   ClassifyC of ML.Classify.Default, takes as input
   1) indep, which is a dataset whose record layout is the same
   as indep of ML.Classify.Logistic.LearnC; and 2) a model
   created by ML.Classify.Logistic.LearnC. The model is used
   to compute the dependent variable for each record of
   indep. A description of the Logistic learning algorithm can
