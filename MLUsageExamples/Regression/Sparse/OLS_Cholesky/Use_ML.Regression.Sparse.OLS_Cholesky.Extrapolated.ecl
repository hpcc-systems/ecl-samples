...
*/
    ...
    sparse_extrapo_height:=sort(ML.Regression.Sparse.OLS_Cholesky(X,Y).Extrapolated(X),id);
    //JOIN Y and sparse_extrapo_height to visually compare
    JOIN(Y,sparse_extrapo_height,LEFT.id=RIGHT.id
